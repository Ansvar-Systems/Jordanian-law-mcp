/**
 * get_jordanian_implementations — Find Jordanian statutes implementing a specific EU directive/regulation
...
export interface GetJordanianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface JordanianImplementationResult {
  document_id: string;
...
export async function getJordanianImplementations(
  db: InstanceType<typeof Database>,
  input: GetJordanianImplementationsInput,
): Promise<ToolResponse<JordanianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as JordanianImplementationResult[];
