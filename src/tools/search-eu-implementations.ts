/**
 * search_eu_implementations — Search EU directives/regulations that have Jordanian implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_jordanian_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  jordanian_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as jordanian_statute_count
  ...
  if (input.has_jordanian_implementation) {
    sql += ' HAVING jordanian_statute_count > 0';
  }
