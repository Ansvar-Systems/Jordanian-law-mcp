#!/usr/bin/env tsx
/**
 * Jordanian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://moj.gov.jo';
const PORTAL_NAME = 'Jordan Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/jordanian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Jordanian Law MCP — Data Freshness Check');
