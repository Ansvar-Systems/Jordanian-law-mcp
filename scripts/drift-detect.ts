#!/usr/bin/env tsx
/**
 * Drift detection for Jordan Law MCP.
 *
 * Checks if upstream Jordan Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Jordan Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'jordanian-law-mcp/1.0 drift-detect' },
      });
